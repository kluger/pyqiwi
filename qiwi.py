...
"""
Python2.7 Qiwi API Wrapper 1.1
by mostm
...
class Person:
	...
	def balance():
		...
		:return:
		"""
		url = "{}/funding-sources/v1/accounts/current".format(endpoint)
	...
	def stat(self, start_date, end_date, operation='ALL', sources=None):
		...
		:return:
		"""
		url = "{}/payment-history/v1/persons/{}/payments/total".format(endpoint, self.number)
...
class Payment:
	...
	def send(self, fields=None, comment=None):
		...
		body = {'id': str(int(1000 * time.time())),
	...
	def cell_payment(self, account, comment=None):
		"""
		Отправить платеж
		:param account: Номер телефона, обязательный параметр
		:type account: str
		:param comment: Комментарий к платежу
		:type comment: str
		:return:
		"""
		pid = self.cell_detect(account)
		self.id = int(pid)
		...
		# self.send(fields=fields, comment=comment)
		url = "{}/sinap/api/v2/terms/{}/payments".format(endpoint, pid)
		if fields is None:
			fields = {'account': str(self.recipient)}
		body = {'id': str(int(1000 * time.time())),
				'sum': {'amount': self.amount,
	...
	def cell_detect(self, number):
		"""
		Получение id оператора
		:param number: Мобильный номер в международном формате.
		:return: pid or False
		"""
		url = "https://qiwi.com/mobile/detect.action"
		params = {'phone': number}
		request = requests.post(url=url, headers=default_headers, params=params)
		try:
			code = request.json()['code']['value']
		except:
			code = '2' # undefined operator
		if code == '0':
...
def commission(pid):
	...
	:return:
	"""
	url = "{}/sinap/providers/{}/form".format(endpoint, pid)
	request = requests.get(url=url, headers=default_headers)
	return request.json()

def cell_detect(number):
	"""
	Получение id оператора
	:param number: Мобильный номер в международном формате.
	:return: pid or False
